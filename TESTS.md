...
| 14 | AGG Reflation leverage re-placed: QQQ 1x + ERX 2x (was QLD 2x + XLE 1x) | Reflation tension study (entry 69): 1x-equity/2x-energy beat 2x/1x on return AND Sortino in both eras (R-month Sortino 0.99/0.85 vs 0.86/0.77); full-tier +0.4pp/0.0pp CAGR, Sortino 1.23→1.26 / 0.92→0.94, maxDD ±2pp. VAGG unchanged (same swap cost 0.6pp both eras). **Matrix v8, 2026-09-09** |
...
| 69 | Reflation tension: long equities AND long energy — which leg breaks, does the equity leg fade past a commodity-extension threshold, and where should R-cell leverage sit | (A) R ends via commodities breaking first (R→G) 58%/67% of the time, equities-first (R→S) 37%/30% — and commodity extension is ~3x higher before R→S exits (+10.4/+7.5%) than before R→G (+2.7/+3.3%), both eras: the oil-shock fear is a real conditional-odds shift. (B) But no threshold where the equity leg turns negative next-month (HIGH-tercile extension: equity +1.3%/+2.2%, both eras) — pre-empting loses: extension-gated variant −2.2/−3.6pp, momentum-directed worst everywhere. The break is handled by the rotation. (C) Leverage placement, era-stable: energy leverage is the efficient leg, equity leverage the inefficient one in R (1x eq/2x energy Sortino 0.99/0.85 vs 3x/2x 0.79/0.64). Full-tier: AGG R → QQQ 1x + ERX 2x = +0.4pp/0.0pp CAGR, Sortino up both eras (1.23→1.26, 0.92→0.94), DD ±2pp — **ADOPTED as matrix v8** (see Adopted #14); VAGG R → QLD+ERX costs 0.6pp both eras for ~0 Sortino — NO |
| 70 | Fragile-month de-levering (breadth / momentum / volume / extension / realized vol / months-since-re-entry → one notch down in G/R) | NO — era-flip, same graveyard as entries 20/50. Modern: crash months carry a fingerprint (SPY <3% above line, 20d vol >16%, weak breadth, first re-entry month 18% crash rate vs 6%) and the composite rule looked spectacular (AGG +1.0pp/Sortino 1.25→1.41/DD −34→−27; VAGG +1.7pp/1.10→1.23/−47→−41). Pre-2007: every rule costs return (composite −0.2/−0.3pp; breadth<70% −2.5/−3.4pp; OR-rule −2.7/−3.9pp) with maxDD unchanged (−44/−62) — the era-2 crashes (Aug 1998 −47.5%/mo, Oct 1987 −42%, Feb 1999 −30%) came from CALM conditions (extension at 57th pctile). Intra-month crash risk under leverage is not forecastable by any monthly ex-ante feature tested; the only era-stable mitigation is the leverage level itself (the ladder). Least-bad: vol>25% → two notches was era-neutral (flat pre-2007) but never touched a drawdown |
| 70b | Fragility shadow (user decision, 2026-09-23) | FILED as informational ledger signal — same treatment as the shadow classifier (58): every row logs `fragility` {ext, rvol20, fragile} and, when fragile, `shadow_delever` (AGG/VAGG one notch down). PRE-REGISTERED PROMOTION TRIGGER: after ≥6 live fragile months, promote to a matrix rule if the de-levered shadow book beat the live AGG/VAGG book in the MAJORITY of those months AND cumulatively; if the live book wins the majority, retire the shadow. Rationale for filing rather than rejecting outright: the modern fingerprint (entered-from-weakness corrections) may be a structural feature of the indexed/automated era — the same hypothesis behind entry 58 — and forward data is the only honest adjudicator |
